const url = "../00_datasets/sgv-12-objects-about-Mann-Frau-counted.json"
...
function preload() {
	loadJSON(url, (res) => {
		...
		data = sortDescending(arr, 'men');
		let maxF = findHighestValue(arr, 'women')
		let maxM = findHighestValue(arr, 'men')
		if (maxF > maxM) {
			maxVal = maxF;
		} else {
			maxVal = maxM
...
function drawBars() {
	...
	for (let posY = 0;posY < width;posY += step) {
		...
		// draw right bar
		let colValW = map(item.women, 1, maxVal, 0, 255);
		fill(250, colValW, 255)
		stroke(0);
		let rSize = map(item.women, 1, maxVal, 0, (width / 2) - rw / 2)
