...
function preload() {
	fetch(url)
		...
		.then(data => {
			...
			delete sortedData["Mann"];
			console.log(sortedData);
			drawData(sortedData);
		})
}

// a function that counts all unique values in an array of string
...
function drawData(data) {
	...
	for (let key in data) {
		let size = map(data[key], 1, 141, 10, width - 230);
...
class Graph {
	constructor(x, y, rad, label, typePos) {
		...
		// constants
		this.res = 18; // the number of points 
